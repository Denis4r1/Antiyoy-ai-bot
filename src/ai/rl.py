...
ENTITY_CHANNELS = {
    "base": 0, "farm": 1, "weakTower": 2, "strongTower": 3,
    "unit1": 4, "unit2": 5, "unit3": 6, "unit4": 7,
}
...
def state_to_tensor(state: dict) -> torch.Tensor:
    ...
    funds = {tr["owner"]: tr["funds"]
             for tr in state.get("territories_data", [])}
    for key, cell in cells.items():
        y, x = map(int, key.split(","))
        ent, own = cell["entity"], cell["owner"]
        if ent in ENTITY_CHANNELS:  t[ENTITY_CHANNELS[ent], y, x] = 1.0
        if own in OWNER_CHANNELS:   t[OWNER_CHANNELS[own], y, x]  = 1.0
        if cell.get("has_moved"):   t[HAS_MOVED_CHANNEL, y, x]    = 1.0
        if own in funds:            t[FUNDS_CHANNEL,  y, x]       = funds[own]/100
    return t
...
class Residual(nn.Module):
    ...
    def forward(self,x): y=torch.relu(self.b1(self.c1(x))); y=self.b2(self.c2(y)); return torch.relu(x+y)
...
class AZNode:
    def __init__(self, st: Optional[GameState], prior: float,
                 par: Optional["AZNode"], act: Optional[Action]):
        self.state, self.prior, self.par, self.act = st, prior, par, act
        self.children: Dict[int,"AZNode"] = {}
        self.v_sum = 0.0; self.n = 0
    def q(self): return 0 if self.n==0 else self.v_sum/self.n
    def u(self,c): return c*self.prior*math.sqrt(self.par.n)/(1+self.n)
    def best(self,c): return max(self.children.values(), key=lambda n:n.q()+n.u(c))

class AlphaZeroMCTS:
    def __init__(self, api: AsyncGameApi, pid: str,
                 net: PolicyValueNet, enc: StateEncoder,
                 sims=400, cpuct=1.4, dev="cpu"):
    ...
    async def initialize(self, s: GameState): self.root=await self._new_root(s)
    async def select(self):
        ...
        return self.root.children[aid].act, {"visit_distribution": {k: v / tot for k, v in visits.items()}}


    async def update(self, aid:int, s:GameState):
        self.root=self.root.children.get(aid) or await self._new_root(s)
        self.root.par=None; self.root.state=s
    ...
    async def _new_root(self, s: GameState):
        ...
        for a in await self.api.get_actions(s):
            root.children[a.action_id] = AZNode(None, pri.get(a.action_id, 1e-3), root, a)
        root.n, root.v_sum = 1, v
        return root

    async def _sim(self,node):
        path=[node]
        while path[-1].children: path.append(path[-1].best(self.c))
        leaf=path[-1]
        if leaf.state is None and leaf.par:
            ...
            term=self.api.is_terminal(leaf.state); win=leaf.state.winner if term else None
        if term: val=1.0 if win==self.pid else -1.0 if win else 0.0
        else:
            pri,val=await self._eval(leaf.state)
            if leaf.n==0:
                for a in await self.api.get_actions(leaf.state):
                    leaf.children[a.action_id]=AZNode(None,pri.get(a.action_id,1e-3),leaf,a)
        for n in reversed(path): n.n+=1; n.v_sum+=val; val=-val

    async def _eval(self,s):
        x=self.enc.encode(s).unsqueeze(0).to(self.dev)
        with torch.no_grad(): log_pi,v=self.net(x)
        ...
        z=sum(pri.values()); pri={k:v/z for k,v in pri.items()}
        return pri,float(v.item())


async def play_game(api_url:str, net:PolicyValueNet,
                    states_dir:Path, train_dir:Path,
                    sims:int, cpuct:float, max_mv:int, game_number: int):
    ...
    for p in pl.values(): await p.initialize(st)
    ...
    for mv in range(max_mv):
        pid = st.players[st.current_player_index]; mcts=pl[pid]
        act,stats=await mcts.select()
        f_train.write(json.dumps({"move":mv,"player":pid,"state":st.model_dump(),
                                  "mcts_stats":stats},ensure_ascii=False)+"\n")
        r=await api.apply_action(st,act)
        if r.state.current_player_index==st.current_player_index:
            et=next((a for a in await api.get_actions(r.state) if a.action_type=="endTurn"),None)
            if et: r=await api.apply_action(r.state,et)
        for p in pl.values(): await p.update(act.action_id,r.state)
        st=r.state; f_states.write(json.dumps(st.model_dump(),ensure_ascii=False)+"\n")
        if r.is_game_over: 
            winner = r.winner
            break

    f_train.write(json.dumps(st.model_dump(),ensure_ascii=False)+"\n")
    f_states.close(); f_train.close(); await api.close()
    return winner
...
class TrainDataset(Dataset):
    def __init__(self, d:Path):
        ...
        for fp in d.glob("*.json"):
            ...
            for e in lines[:-1]:
                s=e["state"]; pi_t=torch.zeros(A_GLOBAL)
                for aid,prob in e["mcts_stats"]["visit_distribution"].items():
                    aid=int(aid); 0<=aid<A_GLOBAL and pi_t.__setitem__(aid,prob)
                z=torch.tensor(1.0 if winner==e["player"] else -1.0 if winner else 0.0)
...
def make_loaders(train_dir:Path,batch:int):
    ...
    if not ds: raise RuntimeError("train-dataset empty")
    n_val=max(1,int(0.1*len(ds))); tr,val=random_split(ds,[len(ds)-n_val,n_val])
    col=lambda b:tuple(torch.stack(x) for x in zip(*b))
    return DataLoader(tr,batch,True,collate_fn=col),DataLoader(val,batch,False,collate_fn=col)


def train(net,tr,val,ep,lr,dev,tb=None):
    net.to(dev); opt=torch.optim.Adam(net.parameters(),lr,weight_decay=1e-4)
    ...
    for e in range(1,ep+1):
        ...
        for x,pi,z in tqdm(tr,f"train {e}/{ep}",leave=False):
            ...
            lp,v=net(x); loss_pi=-(pi*lp).sum(1).mean(); loss_v=F.mse_loss(v,z); loss=loss_pi+loss_v
            opt.zero_grad(); loss.backward(); opt.step()
            tl+=loss.item()*x.size(0); n+=x.size(0)
        net.eval(); 
        with torch.no_grad():
            for x,pi,z in val:
                lp,v=net(x.to(dev)); vl+=F.mse_loss(v.to(dev),z.to(dev)).item()*x.size(0)
        sch.step(vl/n); print(f"[{e}/{ep}] train {tl/n:.4f} | val {vl/n:.4f}")
...
def get_args():
    ...
    p.add_argument("--max-moves",type=int,default=200, help="сколько полуходов максимум играет одна партия")
...
async def main():
    ...
    states_dir = Path(args.states); states_dir.mkdir(parents=True, exist_ok=True)
    train_dir  = Path(args.train);  train_dir.mkdir(parents=True, exist_ok=True)
