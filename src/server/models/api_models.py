...
class CreateRoomResponse(BaseModel):
    room_id: str = Field(..., description="Уникальный ID созданной комнаты", example="room_abc123")
...
class JoinRoomResponse(BaseModel):
    ...
    players_count: int = Field(..., description="Количество игроков в комнате", ge=1, le=8)
...
class RoomStats(BaseModel):
    ...
    lobby_connections: int = Field(..., description="WebSocket подключений к лобби", ge=0)
    game_connections: int = Field(..., description="WebSocket подключений к играм", ge=0)
