...
rooms_created_total = Counter(
    "gameserver_rooms_created_total", "Total number of rooms created"
)

games_started_total = Counter(
    "gameserver_games_started_total", "Total number of games started"
)

websocket_connections_total = Counter(
    "gameserver_websocket_connections_total",
    "Total WebSocket connections",
    ["type"],  # lobby, game
)

websocket_disconnections_total = Counter(
    "gameserver_websocket_disconnections_total",
    "Total WebSocket disconnections",
    ["type"],  # lobby, game
)

api_requests_total = Counter(
    "gameserver_api_requests_total",
    "Total API requests",
    ["method", "endpoint", "status"],
)

errors_total = Counter(
    "gameserver_errors_total", "Total errors", ["type"]
)  # websocket, api, game
...
lobby_connections = Gauge(
    "gameserver_lobby_connections", "Number of active lobby WebSocket connections"
)

game_connections = Gauge(
    "gameserver_game_connections", "Number of active game WebSocket connections"
)

memory_usage_bytes = Gauge(
    "gameserver_memory_usage_bytes", "Memory usage in bytes", ["type"]
)  # rss, vms

# Метрики - гистограммы
request_duration = Histogram(
    "gameserver_request_duration_seconds",
...
class GameServerMetrics:
    ...
    def update_game_metrics(self, rooms_dict, lobby_manager, game_manager):
        ...
        try:
            ...
            # Активные игры
            games_count = sum(
                1 for room in rooms_dict.values() if room.game_controller is not None
            )
            ...
            lobby_count = sum(
                len(connections)
                for connections in lobby_manager.active_connections.values()
            )
...
def get_metrics_response() -> Response:
    ...
    try:
        ...
        return PlainTextResponse(
            content=data.decode("utf-8"), headers={"Content-Type": CONTENT_TYPE_LATEST}
        )
    except Exception as e:
